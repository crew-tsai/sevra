...
// incident fields, the sub-type taxonomy offered when filing an incident, and
// the taxonomy the AI classifies social mentions against.
//
// The database is untouched by any of this. `incidents.incident_type` has a
// CHECK constraint fixing it to the five values below, and `sub_type` is
// unconstrained free text — so every industry can carry its own sub-types
// without a migration, and an industry may relabel a type in the UI while the
// stored value stays the same.
//
// Transportation is listed per *mode* because an airline and a shipping line
// genuinely differ in vocabulary. Other sectors are listed per sector.
