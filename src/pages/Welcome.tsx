...
import { welcomeMessages } from "@/i18n/messages/welcome";
...
// Titles and descriptions live in welcomeMessages, keyed by `to`.
const paths = [
  {
    to: "/sevra",
    icon: Radio,
...
export default function Welcome() {
  ...
  const m = useMessages(welcomeMessages);
  const common = useMessages(commonMessages);
  const locale = useDateLocale();
  const tr = useTranslations("incidents", urgent);
  ...
  useEffect(() => {
    ...
    supabase
      .from("incidents")
      .select("id,title,risk,status,crisis_level,risk_score,updated_at,translations")
    ...
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-6 sm:py-8 space-y-6">
      <header className="space-y-1">
        <p className="text-xs text-muted-foreground">
          {name ? m.welcomeBackName(name) : m.welcomeBack}
        </p>
              ...
              <Link key={i.id} to={`/incidents/${i.id}`} className="group">
                <Card className="px-3 py-2.5 flex items-center gap-3 hover:bg-accent/40 transition-colors">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-medium mt-1 break-words">{tr.text(i, "title")}</p>
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      {m.updated(formatDistanceToNow(new Date(i.updated_at), { addSuffix: true, locale }))}
                    </p>
                  </div>
              ...
              <Card className="p-3 h-full hover:bg-accent/40 transition-colors flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="text-sm font-semibold truncate">{m.paths[p.to].title}</h3>
                  ...
                  <p className="text-xs text-muted-foreground leading-snug mt-0.5">{m.paths[p.to].description}</p>
                </div>
              </Card>
            </Link>
          ))}
