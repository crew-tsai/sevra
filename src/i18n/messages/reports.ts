import { defineMessages } from "@/i18n";

export const reportsMessages = defineMessages({
  en: {
    title: "Reports",
    intro: "Trends across incidents, assets and social intelligence",
    totalIncidents: "Total incidents",
    accuracyTitle: "Is Sevra calling it right?",
    accuracyIntro: "From the verdicts your team recorded on Social Intel, over the last 90 days.",
    accuracyEmpty: "Nobody has corrected a classification yet. On Social Intel, tell Sevra what a mention should have been — that is the only thing this can be measured from.",
    accuracyAgreed: (n: number) => `agreed, out of ${n} judged`,
    accuracyByOne: "disagreed by one band — the mildest kind",
    accuracySerious: "disagreed by more than one band",
    accuracyImplicit: (n: number) =>
      `${n} of these are mentions your team dismissed, counted as "not a crisis". Dismissing something is a verdict, so it is included rather than waiting for anyone to grade mentions by hand.`,
    accuracySplit: (over: number, under: number) =>
      `${over} called worse than your team judged, ${under} called milder.`,
    accuracyOver: "called worse than it was",
    accuracyUnder: "called milder than it was",
    speedTitle: "How fast you speak",
    speedIntro: "From an incident opening to the first communication actually leaving — published, or emailed to a list.",
    speedNothing: "Nothing has been published or sent yet for the incidents in this range, so there is no time to measure.",
    speedMedian: "typical time to the first statement",
    speedFastest: "fastest",
    speedSpoken: (total: number) => `of ${total} incidents have had something go out`,
    speedSilent: (n: number) => `${n} still silent — not counted above`,
    minutesShort: (n: number) => `${n} min`,
    hoursShort: (n: number) => `${n} h`,
    activeNow: "Active now",
    assetsGenerated: "Assets generated",
    socialMentions: "Social mentions",
    approved: "Approved",
    pending: "Pending",
    rejected: "Rejected",
    mentions: "Mentions",
    incidentsPerMonth: "Incidents created per month",
    byRisk: "Stacked by risk level",
    assetsPerMonth: "Assets generated per month",
    byApproval: "Stacked by approval status",
    mentionsPerMonth: "Social mentions per month",
    monitoringVolume: "Volume from continuous monitoring",
    bySource: "Incidents by source",
    allTime: "All-time distribution",
    topTypes: "Top incident types",
    byCategory: "Distribution by category",
    noData: "No data yet.",
  },
  es: {
    title: "Informes",
    intro: "Tendencias de incidentes, contenidos e inteligencia social",
    totalIncidents: "Incidentes totales",
    accuracyTitle: "¿Sevra está acertando?",
    accuracyIntro: "Según los veredictos que tu equipo registró en Social Intel, en los últimos 90 días.",
    accuracyEmpty: "Todavía nadie ha corregido una clasificación. En Social Intel, dile a Sevra qué debería haber sido una mención — es lo único con lo que se puede medir esto.",
    accuracyAgreed: (n: number) => `de acuerdo, de ${n} valoradas`,
    accuracyByOne: "difieren en un nivel — la discrepancia más leve",
    accuracySerious: "difieren en más de un nivel",
    accuracyImplicit: (n: number) =>
      `${n} de estas son menciones que tu equipo descartó, contadas como «no es una crisis». Descartar algo es un veredicto, así que se incluye en vez de esperar a que alguien califique menciones a mano.`,
    accuracySplit: (over: number, under: number) =>
      `${over} calificadas peor de lo que juzgó tu equipo, ${under} más leves.`,
    accuracyOver: "calificadas peor de lo que eran",
    accuracyUnder: "calificadas más leves de lo que eran",
    speedTitle: "Cuánto tardas en hablar",
    speedIntro: "Desde que se abre un incidente hasta que sale la primera comunicación — publicada, o enviada por correo a una lista.",
    speedNothing: "Todavía no se ha publicado ni enviado nada para los incidentes de este rango, así que no hay tiempo que medir.",
    speedMedian: "tiempo habitual hasta la primera declaración",
    speedFastest: "el más rápido",
    speedSpoken: (total: number) => `de ${total} incidentes ya tienen algo publicado`,
    speedSilent: (n: number) => `${n} siguen en silencio — no cuentan arriba`,
    minutesShort: (n: number) => `${n} min`,
    hoursShort: (n: number) => `${n} h`,
    activeNow: "Activos ahora",
    assetsGenerated: "Contenidos generados",
    socialMentions: "Menciones en redes",
    approved: "Aprobados",
    pending: "Pendientes",
    rejected: "Rechazados",
    mentions: "Menciones",
    incidentsPerMonth: "Incidentes creados por mes",
    byRisk: "Agrupados por nivel de riesgo",
    assetsPerMonth: "Contenidos generados por mes",
    byApproval: "Agrupados por estado de aprobación",
    mentionsPerMonth: "Menciones en redes por mes",
    monitoringVolume: "Volumen de la monitorización continua",
    bySource: "Incidentes por origen",
    allTime: "Distribución histórica",
    topTypes: "Tipos de incidente más frecuentes",
    byCategory: "Distribución por categoría",
    noData: "Aún no hay datos.",
  },
});

export const auditMessages = defineMessages({
  en: {
    title: "Audit log",
    intro: "Changes to incident crisis levels and risk scores, and the communication packages Sevra drafted, with previous and new values.",
    filterField: "Filter field",
    allFields: "All fields",
    crisisLevel: "Crisis level",
    riskScore: "Risk score",
    riskLabel: "Risk label",
    mediaPackage: "Communication package",
    statusField: "Status",
    approvalField: "Approval",
    assigneeField: "Assignee",
    publicField: "Public publishing",
    workflowField: "Workflow rule",
    bySevra: "Sevra",
    byPerson: "A person",
    publicValue: { true: "Open", false: "Locked" } as Record<string, string>,
    packageBasis: {
      manual: "Drafted from the communications manual",
      industry_standards: "Drafted from industry standards",
    } as Record<string, string>,
    supportAccess: "Sevra support access",
    supportIntro: "When Sevra staff opened this workspace. Support accounts are listed under Admin → Team & roles, and these entries cannot be edited or deleted.",
    noSupport: "No Sevra staff have opened this workspace.",
    when: "When",
    who: "Who",
    empty: "No changes recorded yet. Future updates to crisis level or risk score will appear here.",
    incident: "Incident",
    field: "Field",
    previous: "Previous",
    new: "New",
    changes: (n: number) => `${n} change${n === 1 ? "" : "s"}`,
  },
  es: {
    title: "Registro de auditoría",
    intro: "Cambios en el nivel de crisis y la puntuación de riesgo de los incidentes, y los paquetes de comunicación que redactó Sevra, con el valor anterior y el nuevo.",
    filterField: "Filtrar por campo",
    allFields: "Todos los campos",
    crisisLevel: "Nivel de crisis",
    riskScore: "Puntuación de riesgo",
    riskLabel: "Nivel de riesgo",
    mediaPackage: "Paquete de comunicación",
    statusField: "Estado",
    approvalField: "Aprobación",
    assigneeField: "Responsable",
    publicField: "Publicación pública",
    workflowField: "Regla de flujo",
    bySevra: "Sevra",
    byPerson: "Una persona",
    publicValue: { true: "Abierta", false: "Bloqueada" } as Record<string, string>,
    packageBasis: {
      manual: "Redactado a partir del manual de comunicación",
      industry_standards: "Redactado según los estándares del sector",
    } as Record<string, string>,
    supportAccess: "Accesos del soporte de Sevra",
    supportIntro: "Cuándo entró el personal de Sevra en este espacio de trabajo. Las cuentas de soporte aparecen en Administración → Equipo y roles, y estas entradas no se pueden editar ni eliminar.",
    noSupport: "Ningún miembro del personal de Sevra ha entrado en este espacio de trabajo.",
    when: "Fecha",
    who: "Quién",
    empty: "Aún no hay cambios registrados. Aquí aparecerán los futuros cambios de nivel de crisis o de puntuación de riesgo.",
    incident: "Incidente",
    field: "Campo",
    previous: "Anterior",
    new: "Nuevo",
    changes: (n: number) => `${n} cambio${n === 1 ? "" : "s"}`,
  },
});
